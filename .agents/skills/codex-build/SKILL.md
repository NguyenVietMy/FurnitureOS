---
name: codex-build
description: "Have Codex implement a concrete work order and have Codex independently inspect the final changes. Preserves the explicit Codex builder choice; use claudex-loop for host-based planning and automatic reviewer selection."
---
...
- In Codex, Codex coordinates and delegates implementation to Codex through the runner's `build --host Codex --builder codex` path. Codex then inspects all changes and independently runs proof checks. Log evidence and limitations. A fresh Codex CLI inspector is also available through the shared runner if useful.
- In Codex, implement in the current host session with its normal tools, then use a fresh Codex inspector through `inspect --host codex --builder codex`. Do not ask Codex to certify its own changes as independent review.
