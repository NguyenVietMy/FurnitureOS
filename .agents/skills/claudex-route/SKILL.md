---
name: claudex-route
description: "Recommend a model and a scoped handoff for a task in Codex or Codex. Use when choosing who should handle a task, seeking a second opinion, getting unstuck, or delegating focused work; execute one handoff when requested."
---
...
- **GPT-6 Astra** (`gpt-6-astra`) and **Codex Fable 5.1** (`Codex-fable-5-1`): candidates for ambiguous work, difficult debugging, or a substantial independent review. From Codex, Fable is a candidate for cross-provider review; from Codex, Astra is a candidate.
- **Codex Sonnet, Opus, or another available model**: retain as options when their task fit, existing context, account access, or the user's preference favors them. Cross-provider delegation is optional.
