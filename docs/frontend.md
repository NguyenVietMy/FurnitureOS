...
The zoned fixtures exercise the public Arrangement contract end to end. The successful fixture renders the exact repaired Placements with a scoped overview camera and keeps the selected fixture, route result and attempt summary visible. The exhausted fixture replaces the canvas with ordinary-language feedback; raw request/access identifiers and solver codes stay in collapsed supporting details. On stacked viewports the Room and stage result remain first and the full arrangement history follows in the rail. Zone geometry, Placement resolution, circulation and repair/drop choices all remain FastAPI-owned. See `docs/zones-circulation.md` for the frozen bounds and deterministic semantics.
