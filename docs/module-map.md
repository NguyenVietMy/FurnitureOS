...
| `modules/units/` | Apartment geometry: what shape the buyer is furnishing. Parses and validates plans, stores them as whole JSONB documents, serves them over HTTP. | `Opening` `Room` `Wall` `UnitPlan` `UnitPlanError` `UnitOut` `parse_unit_plan` `load_seed_unit` `get_unit` `list_unit_slugs` `seed_units` `router` | `tests/test_unit_plan.py` (pure parsing/invariants), `tests/test_units_service.py` (DB), `tests/test_units_api.py` (HTTP) |
...
| Module | Responsibility | Public interface | Test boundary |
|--------|----------------|------------------|---------------|
| `app/` | Routes only. Server components fetch and lay out; no domain logic. | `/`, `/unit/[slug]` | none yet (no rendering tests) |
...
| `modules/units/` | The same slice on the client: fetch a unit, turn walls + openings into solid boxes, render them. | `fetchUnit` `deriveWallPieces` `WallPiece` `UnitScene` and the wire types `Unit` `Wall` `Room` `Opening` `Point` | `geometry.test.ts`, `occlusion.test.ts` — pure functions in vitest's node env |
...
conserved), `scene/coords.ts` the **only** place plan coordinates meet three.js, and
